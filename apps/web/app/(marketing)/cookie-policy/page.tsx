...
export default function CookiePolicyPage() {
  ...
  const fetchPolicy = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await legalApi.getPolicy('cookie');
      setPolicy(data);
    } catch (err) {
    ...
    <div className="container mx-auto max-w-7xl py-16 md:py-24 animate-in fade-in duration-700">
      ...
      <div className="mt-24 pt-12 border-t border-white/10 text-center">
        <p className="text-sm text-neutral-500">
          Have questions about our cookie policy? <a href="mailto:support@recall.ai" className="text-white hover:underline underline-offset-4 font-medium transition-colors">Contact Support</a>
        </p>
      </div>
    </div>
  );
