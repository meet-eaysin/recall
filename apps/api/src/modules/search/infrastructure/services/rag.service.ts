...
export class RagService {
  ...
  async stream(
    ...
    try {
      answer = await resolvedClient.stream({
        messages: this.buildMessages(question, preparation.contextStr, history),
  ...
  private async prepareContext(
    ...
    if (embeddedDocsCount === 0) {
      ...
      return { contextStr: '', sources: [], tokensUsed: 0 };
    }
    ...
    if (documentIds && documentIds.length > 0) {
      filterMust.push({ key: 'documentId', match: { any: documentIds } });
    }
    ...
    for (const result of qdrantResults) {
      ...
      const count = docChunkCounts.get(payload.documentId) ?? 0;
      if (count < 2) {
        docChunkCounts.set(payload.documentId, count + 1);
        selectedChunks.push({
    ...
    const chunkData = await Promise.all(
      selectedChunks.map(async (chunk) => {
        ...
        return { ...chunk, content: chunkDoc?.content ?? '' };
      }),
    );

    const validChunks = chunkData
    ...
    for (const chunk of validChunks) {
      ...
      const author =
        typeof doc.metadata?.author === 'string' ? doc.metadata.author : null;
      const date =
      ...
      if (estimatedTokens + chunkTokens > 6000) break;
  ...
  private async complete(
    ...
    try {
      return await resolvedClient.complete({ messages });
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
